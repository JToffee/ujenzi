...
if (isset ($_POST [ 'postjob'])) {
	...
	$date = clean ($_POST ['jobdate']);
  $email = $_SESSION['user'];
	$client = $_SESSION['login'];

	$sql = "INSERT INTO getworker (category,description,location, payment,email,clientemail,jobdate) VALUES (:cat,:des,:loc,:pay,:email,:client,:jobdate)";
	$query = $conn ->prepare ($sql);
			...
			$query-> bindParam (':jobdate',$date);
      $query-> bindParam (':email',$email);
			$query-> bindParam (':client',$client);
			$query-> bindParam (':jobdate',$date);

			$query-> execute (array(':cat' => $cat,':des' =>$des ,':loc' =>$loc ,':pay' =>$pay ,':email' =>$email,':client' =>$client,':jobdate' =>$date));
...
<html>
<head>
	<title>Ujenzi Kenya Get Worker</title>
	<meta charset="utf-8">
    ...
    <br>
    	...
    	<fieldset>
	          ...
	          <input type="number" placeholder="300" name="payment" required>
						...
						<div class="form-group">
	          <label for="Day"><b>Date of job</b></label>
	          <input type="date" placeholder="25/11/2020" name="jobdate" required>
	          </div>
        </fieldset><br>
